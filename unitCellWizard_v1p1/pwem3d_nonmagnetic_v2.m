function [BETA,WN,epEffMG1,epEffMG2,epEffLW,FF]=pwem3d_nonmagnetic_v2(crystal,pwemParam,sh,th,plotFlag)
    if plotFlag
        figure();
    end
    NSOL=pwemParam.Nsol;
...
%         plot([1:NBETA],WNMG,'Color',[0.5 0.5 0.5],'LineWidth',1);
    ...
    if plotFlag
        ...
        plot(1:NBETA,sqrt(epEffLW),'LineWidth',1)
        grid on; box on;
        %beta axis
        set(gca,'XTick',KT,'XTickLabel',crystal.KL);
        xlabel('Bloch Wave Vector $\vec{\beta}$','Interpreter','LaTex')
